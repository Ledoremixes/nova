...
async function requireNovaOperator(req) {
  ...
  if (!profile || !['admin', 'user'].includes(role) || profile.is_active === false) throw Object.assign(new Error('Operatore Nova non autorizzato.'), { status: 403 })
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      let query = admin.from('nova_packages_catalog').select('*').order('ordine', { ascending: true }).order('nome', { ascending: true })
    ...
    if (req.method === 'POST') {
      const row = { ...body, id: undefined, created_at: undefined, updated_at: new Date().toISOString() }
      const { data, error } = await admin.from('nova_packages_catalog').insert([row]).select('*').single()
      if (error) throw error
    ...
    if (req.method === 'PATCH') {
      ...
      const { data, error } = await admin.from('nova_packages_catalog').update(row).eq('id', id).select('*').single()
      if (error) throw error
      return json(res, 200, data)
    }
