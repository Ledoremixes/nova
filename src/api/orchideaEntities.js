...
function normalizeTeacher(row = {}, table = 'insegnanti') {
  ...
  return {
    ...
    tax_code: row.tax_code || row.codice_fiscale || row.cf || '',
    birth_date: row.birth_date || row.data_nascita || '',
    birth_place: row.birth_place || row.luogo_nascita || '',
    birth_province: row.birth_province || row.provincia_nascita || '',
    residence_city: row.residence_city || row.comune_residenza || '',
    residence_province: row.residence_province || row.provincia_residenza || '',
    residence_address: row.residence_address || row.indirizzo_residenza || '',
    residence_postal_code: row.residence_postal_code || row.cap_residenza || '',
    iban: row.iban || '',
    bank_account_holder: row.bank_account_holder || row.intestatario_iban || '',
    contract_start_date: row.contract_start_date || row.contratto_data_inizio || '',
    contract_end_date: row.contract_end_date || row.contratto_data_fine || '',
    contract_role: row.contract_role || row.contratto_ruolo || '',
    contract_discipline: row.contract_discipline || row.contratto_disciplina || '',
    contract_duties: row.contract_duties || row.contratto_mansioni || '',
    contract_venue: row.contract_venue || row.contratto_sede || '',
    contract_days_turns: row.contract_days_turns || row.contratto_giorni_turni || '',
    contract_time_slots: row.contract_time_slots || row.contratto_fasce_orarie || '',
    contract_estimated_hours: row.contract_estimated_hours || row.contratto_ore_stimate || '',
    contract_notice_days: row.contract_notice_days ?? row.contratto_preavviso_giorni ?? 15,
    contract_compensation_frequency: row.contract_compensation_frequency || row.contratto_periodicita_compenso || 'mensile',
    contract_compensation_tax: row.contract_compensation_tax || row.contratto_compenso_lordo_netto || 'lordo',
    contract_compensation_description: row.contract_compensation_description || row.contratto_descrizione_compenso || '',
    contract_signing_place: row.contract_signing_place || row.contratto_luogo_firma || 'Saronno',
    contract_competent_court: row.contract_competent_court || row.contratto_foro || '',
    created_at: row.created_at || null,
  }
}
...
function teacherPayloadForTable(payload, table) {
  ...
  if (table === 'teachers' || table === 'teachers_nova') {
    return {
      ...
      birth_province: payload.birth_province?.trim() || null,
      residence_city: payload.residence_city?.trim() || null,
      residence_province: payload.residence_province?.trim() || null,
      residence_address: payload.residence_address?.trim() || null,
      residence_postal_code: payload.residence_postal_code?.trim() || null,
      iban: payload.iban?.replace(/\s+/g, '').toUpperCase() || null,
      bank_account_holder: payload.bank_account_holder?.trim() || null,
      contract_start_date: payload.contract_start_date || null,
      contract_end_date: payload.contract_end_date || null,
      contract_role: payload.contract_role?.trim() || null,
      contract_discipline: payload.contract_discipline?.trim() || null,
      contract_duties: payload.contract_duties?.trim() || null,
      contract_venue: payload.contract_venue?.trim() || null,
      contract_days_turns: payload.contract_days_turns?.trim() || null,
      contract_time_slots: payload.contract_time_slots?.trim() || null,
      contract_estimated_hours: payload.contract_estimated_hours?.trim() || null,
      contract_notice_days: payload.contract_notice_days === '' || payload.contract_notice_days === null || payload.contract_notice_days === undefined ? 15 : Number(payload.contract_notice_days || 15),
      contract_compensation_frequency: payload.contract_compensation_frequency || 'mensile',
      contract_compensation_tax: payload.contract_compensation_tax || 'lordo',
      contract_compensation_description: payload.contract_compensation_description?.trim() || null,
      contract_signing_place: payload.contract_signing_place?.trim() || 'Saronno',
      contract_competent_court: payload.contract_competent_court?.trim() || null,
    }
  }
  ...
  return {
    ...
    codice_fiscale: payload.tax_code?.trim() || null,
    data_nascita: payload.birth_date || null,
    luogo_nascita: payload.birth_place?.trim() || null,
    provincia_nascita: payload.birth_province?.trim() || null,
    comune_residenza: payload.residence_city?.trim() || null,
    provincia_residenza: payload.residence_province?.trim() || null,
    indirizzo_residenza: payload.residence_address?.trim() || null,
    cap_residenza: payload.residence_postal_code?.trim() || null,
    iban: payload.iban?.replace(/\s+/g, '').toUpperCase() || null,
    intestatario_iban: payload.bank_account_holder?.trim() || null,
    contratto_data_inizio: payload.contract_start_date || null,
    contratto_data_fine: payload.contract_end_date || null,
    contratto_ruolo: payload.contract_role?.trim() || null,
    contratto_disciplina: payload.contract_discipline?.trim() || null,
    contratto_mansioni: payload.contract_duties?.trim() || null,
    contratto_sede: payload.contract_venue?.trim() || null,
    contratto_giorni_turni: payload.contract_days_turns?.trim() || null,
    contratto_fasce_orarie: payload.contract_time_slots?.trim() || null,
    contratto_ore_stimate: payload.contract_estimated_hours?.trim() || null,
    contratto_preavviso_giorni: payload.contract_notice_days === '' || payload.contract_notice_days === null || payload.contract_notice_days === undefined ? 15 : Number(payload.contract_notice_days || 15),
    contratto_periodicita_compenso: payload.contract_compensation_frequency || 'mensile',
    contratto_compenso_lordo_netto: payload.contract_compensation_tax || 'lordo',
    contratto_descrizione_compenso: payload.contract_compensation_description?.trim() || null,
    contratto_luogo_firma: payload.contract_signing_place?.trim() || 'Saronno',
    contratto_foro: payload.contract_competent_court?.trim() || null,
    updated_at: new Date().toISOString(),
  }
}
